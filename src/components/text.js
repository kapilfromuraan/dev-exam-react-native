...
export const TextLarge = ({ containerStyle, children, style, onPress, align, fullWidth }) => <TouchableOpacity style={[fullWidth ? { width: '100%' } : null, containerStyle]} disabled={!onPress} onPress={onPress}><Text style={[{ fontSize: 20, color: colors.white, marginTop: Platform.OS === 'android' ? 3 : 0, fontFamily: fonts.medium, textAlign: align || 'center' }, style]}>{children}</Text></TouchableOpacity>
